...
//
// opMu serializes Tick, external HandleRequest, ConfigSnapshot,
// ControlActive / InotifyActive, and the bulk of Shutdown so Config/engine/
// scanner mutations and control/inotify pointer teardown cannot race concurrent
// HTTP ops or status helpers. ServeReady runs only under Tick while opMu is
// held; the control Handler is handleRequestLocked (no re-lock) to avoid
// deadlock. config_set live-applies doReload once and does not also
// RequestReload for the next tick. Shutdown closes HTTP without opMu first
// (handlers need opMu), then takes opMu for control/inotify/unmount/store
// teardown. Readers of s.control / s.inotify must hold opMu (same as Shutdown
// nils). APIBackend.Config returns ConfigSnapshot (deep copy), not the live
// s.Config pointer.
//
