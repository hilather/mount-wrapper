...
| Outer nonsolid cache | Scope `outer`/`all`: mount path calls `EnsureNonsolidCachedCopy` — solid → CLI extract + `a -ms=off` under content-keyed cache dest; exclusive `{cacheKey}.lock` flock serializes concurrent populate; non-solid only when `7z l` succeeds and `Solid != +` (list fail/empty fail-closed); post-populate `FlattenMinOKSize` floor; post-populate re-list of dest must pass `nonsolidCacheHit` (still-solid / list fail / encrypted → remove dest + clear error; **no stream-flatten** fallback); encrypted list/extract → `Encrypted7zMessage`; leftover `*.nonsolid.partial` / `*.work` cleaned before populate. **Cache hit** (fast path or under-lock re-check): if dest is usable and the convert sidecar is missing, best-effort write size-only metadata (`original=source Stat`, `converted=dest Stat`, method `outer-nonsolid-cli`, **duration omitted** — never invent duration on a hit; no thrash-rewrite when sidecar already exists). Populate still writes full sidecar with measured duration. On successful resolve to a cache path (`mountArchive != archivePath`), the mount **claim** `Transition` persists `convert_source_size_bytes` / `convert_duration_seconds` when those store columns are still nil: prefer `convert.ReadConvertMetadata(mountArchive)` (sidecar from populate or hit backfill); fall back to `Stat(source)` for original size only when sidecar still missing/unreadable — **no invented duration** without a timed-populate sidecar field |

**Engine convert order** (parity with Python `_run_convert`): archiveconverter (if available / `.7z` / not zip-repack) → zip repack → flatten. Success updates `archive_path` + fingerprint, preserves `archive_basename` when the product lives under `archiveconverter_output_dir/{archive_id}.7z` (operator-visible name unchanged), clears a stale `mount_path` when basename changes, leaves `converting` → `discovered` (also accepts recovery from `mount_failed` / `index_failed` after a race), then continues to mount/index. Convert jobs stay registered until the post-convert store transition succeeds; `BeginMount` skips archives in `converting` or with an active convert job, and reuses `ExistingConvertedPath` instead of starting a duplicate convert. Failure → `index_failed` / `mount_failed` with `last_error`. Outer/all cache populate runs at **mount** start (not the convert job), like Python `resolve_mount_archive_path`.
...
| Outer cache | CLI extract+repack only (no stream-repack / stream-flatten); exclusive flock on `{cacheKey}.lock` with re-check hit inside lock; fail-closed list + size floor + post-populate solid verify (`nonsolidCacheHit`; still-solid/unlistable dest removed) + leftover partial/work cleanup; nested `.7z` members not expanded in outer cache (child env still used for nested when scope allows); hit without sidecar writes size-only metadata (duration still only from timed populate); store convert columns filled on mount claim from cache sidecar (or source size fallback) so status/SPA stay durable without relying only on live sidecar reads |
...
**Mount process memory:** when `state.mount_pid` is set (after `MarkMounted` / live FUSE child), metrics sample the child **RSS** (Linux `/proc/<pid>/status` `VmRSS`; Darwin `ps -o rss=`) and optional **peak RSS** (Linux `VmHWM`). Fields: per-archive `mount_rss_bytes` / `mount_rss_peak_bytes` / `mount_pid`; summary `total_mount_rss_bytes`, `total_mount_rss_peak_bytes`, `archives_with_mount_rss`. Samples run on metrics compute (TTL cache); not a durable SQLite column. Prometheus: `mount_wrapper_mount_rss_bytes` (+ peak + count). SPA SavingsBar second line + table column.

**Implemented:** pure formulas + FS/SQLite deep/shallow index analysis + mount walk fallback/promotion + process RSS sampling; unit tests with map fakes and synthetic nested indexes (flatten double-count, opaque-only, mount promote, mount RSS). Cache isolates index-first vs `prefer_mount` so neither path returns the other's extracted sizes within TTL; `no_cache` recomputes and refreshes only the matching key; `Invalidate(archive_id)` drops both variants.
...
**Wired:** production Metrics collector on serve + control `metrics` / status `include_sizes`. CLI `status [--json] [--sizes]` (7.2): default multi-line human via `status.FormatHuman`; `--sizes` without `--json` appends a human sizes appendix (`metrics_summary` + per-archive metrics) rather than forcing JSON; `--json` / `--sizes --json` dump the full payload. HTTP `/api/status` + SSE snapshot/deltas (Phase 9). SPA reactive table still Phase 10.
...
| Darwin | **`launchd_agent`** (`launchctl list` / `print` for `com.hilather.mount-wrapper`; clear list/print loaded shape → info; not loaded / missing tool / unclassifiable → warn; injectable; never hard-fail) |
